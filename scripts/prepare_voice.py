...
# Production mode: local VieNeu only, no external ZeroGPU/API dependency.
# INT8 + single ONNX thread keeps memory/CPU usage as low as possible
# while preserving the exact configured VieNeu voice.
...
print(f'Prepared {len(scenes)} scenes, {len(full)/SR:.2f}s, voice={voice}, backend=local-int8')
